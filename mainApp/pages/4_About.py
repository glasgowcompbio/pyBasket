...
st.write("This App has been developed by Marina Flores Payan as the final project for the MSc in Bioinformatics at the University of Glasgow.")
st.write("For more information about pyBasket, visit the pyBasket [page](%s)" % url_pybasket)

st.write("##### Web framework")
st.write("pyBasket app: [repository](%s). --- For full scripts and code underlying the pyBasket app." % repository)
st.write(
    "The web interface of the pyBasket pipeline has been implemented based on the open source Python's Streamlit library ([link](%s))" % url_streamlit)
