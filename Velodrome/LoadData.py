...
def mapping(data_file, mapping_table, exclude_many_to_one=True):
    ...
    mapping_table - pandas dataframe with "ensg_v99" and "entrez_v99" columns --
    Ensembl and Entrez gene ids for Ensembl v99.
    exclude_many_to_one - whether to exclude genes which Entrez ids mapped to a non-unique Ensemble.
    If False, for each Ensembl id, sum of rows of with correponding Entrez ids is reported
    (makes sense for expression data)'''
    # reading inputs
    ...
    mapping_table = mapping_table.loc[mapping_table["entrez_v99"].isin(
        set(mapping_table["entrez_v99"].values).intersection(set(data.index.values))), :]
    ...
    entrez_one_to_many = set(
        entrez_one_to_many.loc[entrez_one_to_many["ensg_v99"] > 1, :].sort_values(
            "ensg_v99").index.values)
    # print("one-to-many (will be dropped):")
    # print("\t%s Entrez ids -> %s Ensemble ids"%(len(entrez_one_to_many),
    #                                             mapping_table.loc[mapping_table["entrez_v99"].
    #                                             isin(entrez_one_to_many),:].shape[0]))

    # identifying genes mapped as many-to-one:
    many_to_one = \
        mapping_table.loc[~mapping_table["entrez_v99"].isin(entrez_one_to_many)].groupby(
            "ensg_v99")[
            ["entrez_v99"]].agg('count')
    many_to_one = set(
        many_to_one.loc[many_to_one["entrez_v99"] > 1, :].sort_values("entrez_v99").index.values)
    #     if exclude_many_to_one:
    #         print("many-to-one (will be dropped):")
    #     else:
    #         print("many-to-one (will be summed and mapped):")
    entrez_many_to_one = set(
        mapping_table.loc[mapping_table["ensg_v99"].isin(many_to_one), "entrez_v99"].values)
    #     print("\t%s Entrez ids -> %s Ensemble ids"%(len(entrez_many_to_one),len(many_to_one)))
    many_to_one_mapping = mapping_table.loc[
        mapping_table["ensg_v99"].isin(many_to_one), ["entrez_v99", "ensg_v99"]]
    ...
    if not exclude_many_to_one:
        ...
        many_to_one_mapping = \
            many_to_one_mapping.groupby("ensg_v99")[["entrez_v99"]].agg(list).to_dict()[
                'entrez_v99']
        d2_mapped = {}
...
def prep_train_data(args):
    ...
    genes_1 = pd.read_csv(args.data_root + "gene_list_4Feb.mapped_to_ens99.tsv", sep="\t",
                          index_col=None)
    genes_2 = pd.read_csv(args.data_root + "gene_list_4Feb.mapped_to_ens99_part2.tsv", sep="\t",
                          index_col=None)
    ...
    mapping_table = pd.read_csv(args.data_root + "Entrez_to_Ensg99.mapping_table.tsv", sep="\t",
                                index_col=0)
    ...
    if drug == "Erlotinib":
        ...
        GSE33072R = pd.read_csv(args.data_root + "GSE33072_response.Erlotinib.tsv", sep="\t",
                                index_col=0, decimal=",")
        PDX_ErloR = pd.read_csv(args.data_root + "PDX_response.multi-OMICS.Erlotinib.tsv",
        ...
        return X_tr, Y_tr, gCSI_exprs_drug.values, gCSI_aac_drug.values, GSE33072.values, \
               GSE33072R, PDX_Erlotinib.values, PDX_ErloR, X_U
    ...
    if drug == "Docetaxel":
        ...
        GSE25065DR = pd.read_csv(args.data_root + "GSE25065_response.Docetaxel.tsv", sep="\t",
                                 index_col=0, decimal=",")
        ...
        return X_tr, Y_tr, gCSI_exprs_drug.values, gCSI_aac_drug.values, GSE25065D.values, \
               GSE25065DR, X_U
    ...
    if drug == "Gemcitabine":
        ...
        TCGA_GemR = pd.read_csv(args.data_root + "TCGA_response.Gemcitabine.tsv", sep="\t",
                                index_col=0, decimal=",")
        PDX_GemR = pd.read_csv(args.data_root + "PDX_response.multi-OMICS.Gemcitabine.tsv",
                               sep="\t", index_col=0, decimal=",")
        ...
        return X_tr, Y_tr, gCSI_exprs_drug.values, gCSI_aac_drug.values, TCGA_Gemcitabine.values, \
               TCGA_GemR, PDX_Gemcitabine.values, PDX_GemR, X_U
    ...
    if drug == "Paclitaxel":
        ...
        PDX_PacR = pd.read_csv(args.data_root + "PDX_response.multi-OMICS.Paclitaxel.tsv",
                               sep="\t", index_col=0, decimal=",")
        GSE25065PR = pd.read_csv(args.data_root + "GSE25065_response.Paclitaxel.tsv", sep="\t",
                                 index_col=0, decimal=",")
        ...
        return X_tr, Y_tr, gCSI_exprs_drug.values, gCSI_aac_drug.values, GSE25065P.values, \
               GSE25065PR, PDX_Paclitaxel.values, PDX_PacR, X_U
