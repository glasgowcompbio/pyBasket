...
def main():
    ...
    train_arg_parser.add_argument('--best_params', type=int, default=0, help='use best parameters from paper')
    ...
    if args.best_params == 1: # best parameters from paper
        ...
        if args.drug == "Docetaxel":
            args.save_logs = './Velodrome/%s/logs/' % args.drug
            args.save_models = './Velodrome/%s/models/' % args.drug
            args.save_results = './Velodrome/%s/results/' % args.drug
            args.seed = 42
            args.epoch = 10
            args.bs = 65
            args.ldr = 0.1
